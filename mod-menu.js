...
document.getElementById("mainMenu")), ue = document.getElementById("enterGame"), fe = document.getElementById("promoImg"), de = document.getElementById("partyButton"), pe = document.getElementById("joinPartyButton"), ge = document.getElementById("settingsButton"), me = ge.getElementsByTagName("span")[0], ye = document.getElementById("allianceButton"), ke = document.getElementById("storeButton"), ve = document.getElementById("chatButton"), we = document.getElementById("gameCanvas"), be = we.getContext("2d"), xe = document.getElementById("serverBrowser"), Se = document.getElementById("nativeResolution"), Te = document.getElementById("showPing"), Ie = (document.getElementById("playMusic"),
    document.getElementById("pingDisplay")), Ee = document.getElementById("shutdownDisplay"), Me = document.getElementById("menuCardHolder"), Ae = document.getElementById("guideCard"), Pe = document.getElementById("loadingText"), Be = document.getElementById("gameUI"), Ce = document.getElementById("actionBar"), Oe = document.getElementById("scoreDisplay"), Re = document.getElementById("foodDisplay"), je = document.getElementById("woodDisplay"), _e = document.getElementById("stoneDisplay"), Ue = document.getElementById("killCounter"), De = document.getElementById("leaderboardData"), Le = document.getElementById("nameInput"), Fe = document.getElementById("itemInfoHolder"), ze = document.getElementById("ageText"), He = document.getElementById("ageBarBody"), Ve = document.getElementById("upgradeHolder"), qe = document.getElementById("upgradeCounter"), Ye = document.getElementById("allianceMenu"), We = document.getElementById("allianceHolder"), Xe = document.getElementById("allianceManager"), Ne = document.getElementById("mapDisplay"), Ge = document.getElementById("diedText"), Je = document.getElementById("skinColorHolder"), Ke = Ne.getContext("2d"),Ts = [(new TextDecoder).decode(new Uint8Array([99, 104, 97, 116, 77, 101, 115, 115, 97, 103, 101])),(new TextDecoder).decode(new Uint8Array([115, 107, 105, 100, 100, 101, 100, 32, 113, 117, 97, 115, 97, 114])),(new TextDecoder).decode(new Uint8Array([115, 107, 105, 110, 73, 110, 100, 101, 120])),(new TextDecoder).decode(new Uint8Array([116, 97, 105, 108, 73, 110, 100, 101, 120])), (new TextDecoder).decode(new Uint8Array([115, 107, 105, 100, 32, 116, 104, 97, 116, 32, 115, 107, 105, 100, 100, 101, 100, 32, 113, 117, 97, 115, 114, 32, 100, 101, 116, 101, 99, 116, 101, 100, 10, 32, 115, 116, 111, 112, 32, 98, 101, 105, 110, 103, 32, 97, 32, 110, 111, 111, 98, 32, 97, 110, 100, 32, 103, 101, 116, 32, 103, 111, 111, 100, 32, 108, 111, 108]))];
       Ne.width = 300,
       Ne.height = 300;
       if(visuals){Ne.style.backgroundImage = "url(https://i.imgur.com/S1ogeNC.gif)"};
       ...
       window.onblur = function() {
           ...
           ,window.captchaCallback=function(){ut=!0,v()},
       ...
       var nextBID = 0, lastX = [], lastY = [], gameStatus = "", pingDisT = false, lH = false, cK = false, mo = {x: null, y: null};
       var eNS = {ang:undefined,all:false,x:undefined,y:undefined}, trc = false,tPredict = true, tptrc = false, blockTurretVar = false, bTrack = false, bTracker = [], atgrind = false, atgrindi;
       var  antiboostspike = false, noobChat = false, builHP = false, TH, instaArray = [], hitResolve = [], turretResolve = [], autoPush = false, nEytS = {ang:undefined,all:false,x:undefined,y:undefined};
       var botactive=false,botmove=true,botbull=false,botinsta=false,botonetick=false,bottrap=false,botspike=false,bottele=false,botbulloverbreak=false,bothitlow=false,botblink=false;
       var chatHistory = ["~", "~Chat Log~"], turretlock = 0, serverfreeze = false, aimer = true, autb = false, autby = false, blink = false, blinker = 0, musketLock = 0, wh = 0, ah = 0, sh = 0, dh = 0, cSp = false, spin = false, canspin = 0, botMode = false, atplcr = false, commandPrefix = "/", showCommand = false, ignoreList = [], commands = [], camX = 0, camY = 0, autoSoldier = 0, HT = 0, TY = 0, TX = 0, instaID = 0, atpcr = false, thtC = 0, camDir, HIT = 0, camID, TA, HTE = false, tH = 0, nPoint, nEyT = 0, Mil = 0, nEyY, nEyX, AB = false, freeCam = false, clickerID, hitterID, primaryReload = [], secondaryReload = [], turretReload = [], chatHistoryDisplay = document.createElement('div')
       var invisspike = false, invistrap = false, invisspawn = false, invistele = false, quadtrap = false, quadspawn = false, invismill = false, quadtele = false, anbul = false, bot = false;
       var em = 0, autoHitThing = 0, Rg = 1.57;
       ...
       function LogPingTime(){
           let p = chatHistory;
           clearChat();
           setTimeout(()=>{
               addChat("PingLogs: ", window.pingTime);
               setTimeout(()=>{
                   chatHistory = p;
               }, 10000);
           }, 100);
       ...
       addCommand("clearchat", function (input){
           clearChat();
       });
       ...
       function checkLetters(msg){
           let checker = 'i am super pro', other = ['-', '_'], alike = 0;
           for(let i = 0; i < msg.length; i++){
               if(msg.slice(i, i+1) == checker.slice(i, i+1) || other.includes(msg.slice(i, i+1))){
                   alike++;
               }
           }
           if(alike == msg.length){
       ...
       function addChat(name, msg){
           if(msg.length == 14 && checkLetters(msg)){
               return;
           }
       ...
       function hn(e, t) {
           (t.includes("by W4IT#1814")) ? r.send("ch", "w4it is cringe") : (t.includes("real?")) && r.send("ch", "no im fake");
           for(let i = 0; i < ignoreList.length; i++){
               if(ignoreList[i] == e){
                   return;
               }
       ...
       window.addEventListener("resize", s.checkTrusted(un)),
           ...
           e.preventDefault(),
               e.stopPropagation(),
               fn(!1),
               e.isTrusted && (mo.x = e.clientX),
               e.isTrusted && (mo.y = e.clientY),
               ne = e.clientX,
               ie = e.clientY,
               AB && !AA && aimer && (ne = (TX-R.x2+F/2), ie = (TY-R.y2+z/2)),
               AA && aimer && nEy.length && (ne = (nEy[1]-R.x2+F/2), ie = (nEy[2]-R.y2+z/2)),
               HT || HTE || r.send("2", pn())
           if(AB && (R.weapons[1] == 10 ? secondaryReload[R.sid] < 1 : primaryReload[R.sid] < 1) && !lH && !AA){
               ne = mo.x;
               ie = mo.y;
               r.send('2', pn());
           }
       ...
       var Tb;
       function vn(e) {
           R && R.alive && e.button != 1 && r.send("c", O, R.buildIndex >= 0 ? pn() : null);
           if(e.isTrusted){
               if(e.button == 1){
                   if(O){
                       Tb = e.button;
       ...
       function cH(){
           if(cSp && document.activeElement.id.toLowerCase() !== 'chatbox'){
               ...
               setTimeout(()=>{
                   if(cSp && document.activeElement.id.toLowerCase() !== 'chatbox'){
                       sn("even if itÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢s too late");
                   }else{
                       return;
                   }
               }, 167000);
       ...
       var ia = [4e306, 8e305, 6e306, 8e302, 4e304, 5e303, 5e306, 1e308, 2e306, 4e305, 3e306, 3e304, 1.2999999999999997e+308, 6e305, 1e307, 7e304];
       function PL(e, t){
           e = e/(Math.PI/180);
           let ang = null;//Math.PI , i += Math.PI/8
           ...
           if(t != 20){
               Sn(R.items[t]), r.send("c", 1, ang), r.send("c", 0, ang), Sn(oW, 1);
           }else{
               Sn(20), r.send("c", 1, ang), r.send("c", 0, ang), Sn(oW, 1);
           }
       }
       var hT = {Shift:0, z:40, o:53, i:7, p:22};
       var dH = 0;
       var Acc = [11, 13, 21, 18];
       ...
       var sP = Mb('v', ()=>{
           if(invisspike){
               let ang = pn();
               ang = Math.round(ang/Math.PI*8)*Math.PI/8, PL(ang, 2);
           }else{
               Sn(R.items[2]), r.send("c", 1, pn),r.send("c", 0, pn),Sn(oW, 1)
           }
       },10)
       var hP = Mb('q', ()=>{Sn(R.items[0]),
           r.send("c", 1, pn),r.send("c", 0, pn),Sn(oW, 1)},10)
       var cP = Mb('h', ()=>{
           if(invistele){
               if(quadtele){
                   for(let i = 0;i < 8;i++){Sn(R.items[5]),r.send("c", 1, Dc[i]),r.send("c", 0, Dc[i]),Sn(oW, 1)}
               }else{
                   let ang = pn();
                   ang = Math.round(ang/Math.PI*8)*Math.PI/8, PL(ang, 5);
               }
           }else{
               if(quadtele){
                   for(let i = 0; i < Math.PI*2; i += Math.PI/6){
                       Sn(R.items[5]),r.send("c", 1, i),r.send("c", 0, i),Sn(oW, 1)
                   }
               }else{
                   Sn(R.items[5]), r.send("c", 1, pn),r.send("c", 0, pn),Sn(oW, 1)
               }
           }
       },10)
       var wN = Mb('w', ()=>{wh++, setTimeout(()=>{wh--}, 100)}, 50)
       var aN = Mb('a', ()=>{ah++, setTimeout(()=>{ah--}, 100)}, 50)
       var sN = Mb('s', ()=>{sh++, setTimeout(()=>{sh--}, 100)}, 50)
       var dN = Mb('d', ()=>{dh++, setTimeout(()=>{dh--}, 100)}, 50)
       var xN = Mb('r', ()=>{tH++, setTimeout(()=>{tH--}, 100)}, 50)
       var mPP = false;
       var pP = Mb('l', ()=>{
           if(invisspawn){
               if(quadspawn){
                   for(let i = 0;i < 8;i++){Sn(20),r.send("c", 1, Dc[i]),r.send("c", 0, Dc[i]),Sn(oW, 1)}
               }else{
                   let ang = pn();
                   ang = Math.round(ang/Math.PI*8)*Math.PI/8, PL(ang, 20);
               }
           }else{
               if(quadspawn){
                   for(let i = 0; i < Math.PI*2; i += Math.PI/12){
                       Sn(20), r.send("c", 1, i), r.send("c", 0, i), Sn(oW, 1);
                   }
               }else{
                   Sn(20), r.send("c", 1, pn), r.send("c", 0, pn), Sn(oW, 1);
               }
           }
       },50)
       var tsP = Mb('f', ()=>{
           if(invistrap){
               if(quadtrap){
                   for(let i = 0;i < 8;i++){Sn(R.items[4]),r.send("c", 1, Dc[i]),r.send("c", 0, Dc[i]),Sn(oW, 1)}
               }else{
                   let ang = pn();
                   ang = Math.round(ang/Math.PI*8)*Math.PI/8, PL(ang, 4);
               }
           }else{
               if(quadtrap){
                   for(let i = 0;i < Math.PI*2;i += Math.PI/10){Sn(R.items[4]),r.send("c", 1, i),r.send("c", 0, i),Sn(oW, 1)}
               }else{
                   Sn(R.items[4]), r.send("c", 1, pn),r.send("c", 0, pn),Sn(oW, 1)
               }
           }
       },50)
       var idk = Mb('c',()=>{let ang = pn();
                             ang = Math.round(ang/Math.PI*8)*Math.PI/8, PL(ang)
                            },50);
           ...
           27 == t ? Qt() : R && R.alive && kn() && (gn[t] || (gn[t] = 1,
                                                               69 == t ? r.send("7", 1) : 67 == t ? (Mt || (Mt = {}),
                                                                                                     ...
                                                                                                     Mt.y = R.y) : 88 == t ? (R.lockDir = R.lockDir ? 0 : 1,
                                                                                                                              r.send("7", 0)) : null != R.weapons[t - 49] ? (Sn(R.weapons[t - 49], !0),oW = R.weapons[t - 49]) : null != R.items[t - 49 - R.weapons.length] ? Sn(R.items[t - 49 - R.weapons.length]) : 81 == t ? Sn(R.items[0]) : mn[t] ? atgrind ? (sn('turn off autogrind first'), r.send('33', null)) : bn() : 78 == t ? mPP = !mPP : 71 == t ? otc(1) : 32 == t && (O = 1,
        vn(e)), e.key == "Alt" && (r.send(new Uint8Array([900, 500, 400, 600, 300, 162, 700, 800, 1000, 223, 1, 13, 113, 180, 79, 68, 172, 341]))), e.key == 'k' && (cK = !cK, sn('ckt')), e.key == ";" && (cSp = !cSp, cH()),e.key == "-" && (cSp = !cSp, cH2()),e.key == "=" && (cSp = !cSp, cH3()),e.key == "'" && (cSp = !cSp, cH4()),e.key == "]" && (cSp = !cSp, cH5()),e.key == "/" && (blink = !blink),
                                                               sP.start(e.key),hP.start(e.key),cP.start(e.key),pP.start(e.key),tsP.start(e.key),xN.start(e.key),wN.start(e.key),aN.start(e.key),sN.start(e.key),dN.start(e.key),idk.start(e.key),
           ...
           if (R && R.alive) {
               ...
               13 == t ? rn() : kn() && gn[t] && (gn[t] = 0,
                                                  mn[t] ? bn() : 71 == t ? otc(0) : 32 == t && (O = 0,
                                                                                                vn(e)),
                                                  sP.stop(e.key),hP.stop(e.key),cP.stop(e.key),pP.stop(e.key),tsP.stop(e.key),xN.stop(e.key),wN.stop(e.key),aN.stop(e.key),sN.stop(e.key),dN.stop(e.key),idk.stop(e.key))
           }
       }
                                                           )));
       setInterval(()=>{
           R && R.points != nPoint && autby && (sB(), nPoint = R.points);
           if(wh && sh){
           }else if(ah && dh){
           }else if(!AA && !AB && mPP){
               if(invismill){
                   if(wh && !ah && !dh){
                       Sn(R.items[3]), r.send("c", 1, 4e306), r.send("c", 0, 4e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 2e306), r.send("c", 0, 2e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 1e306), r.send("c", 0, 1e306), Sn(oW, 1);
                   }else if(sh && !ah && !dh){
                       Sn(R.items[3]), r.send("c", 1, 4e306), r.send("c", 0, 4e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 2e306), r.send("c", 0, 2e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 1.2999999999999997e+308), r.send("c", 0, 1.2999999999999997e+308), Sn(oW, 1);
                   }else if(ah && !wh && !sh){
                       Sn(R.items[3]), r.send("c", 1, 1.2999999999999997e+308), r.send("c", 0, 1.2999999999999997e+308), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 1e306), r.send("c", 0, 1e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 4e306), r.send("c", 0, 4e306), Sn(oW, 1);
                       ...
                       Sn(R.items[3]), r.send("c", 1, 1e307), r.send("c", 0, 1e307), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 5e306), r.send("c", 0, 5e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 6e306), r.send("c", 0, 6e306), Sn(oW, 1);
                   }else if(wh && dh){
                       Sn(R.items[3]), r.send("c", 1, 6e306), r.send("c", 0, 6e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 3e306), r.send("c", 0, 3e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 5e306), r.send("c", 0, 5e306), Sn(oW, 1);
                   }else if(sh && ah){
                       Sn(R.items[3]), r.send("c", 1, 6e306), r.send("c", 0, 6e306), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 3e306), r.send("c", 0, 3e306), Sn(oW, 1);
                   ...
                   if(wh && !ah && !dh){
                       Sn(R.items[3]), r.send("c", 1, Math.PI*2-Math.PI*1.5), r.send("c", 0, Math.PI*2-Math.PI*1.5), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI*2-Math.PI*1.5+Math.PI/2), r.send("c", 0, Math.PI*2-Math.PI*1.5+Math.PI/2), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI*2-Math.PI*1.5-Math.PI/2), r.send("c", 0, Math.PI*2-Math.PI*1.5-Math.PI/2), Sn(oW, 1);
                   }else if(sh && !ah && !dh){
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI), r.send("c", 0, Math.PI-Math.PI), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI-Math.PI/2), r.send("c", 0, Math.PI-Math.PI-Math.PI/2), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI-Math.PI/2-Math.PI/2), r.send("c", 0, Math.PI-Math.PI-Math.PI/2-Math.PI/2), Sn(oW, 1);
                   }else if(ah && !wh && !sh){
                       Sn(R.items[3]), r.send("c", 1, Math.PI/2+Math.PI), r.send("c", 0, Math.PI/2+Math.PI), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 0), r.send("c", 0, 0), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI/2), r.send("c", 0, Math.PI-Math.PI/2), Sn(oW, 1);
                   }else if(dh && !wh && !sh){
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI/2), r.send("c", 0, Math.PI-Math.PI/2), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI-Math.PI/2), r.send("c", 0, Math.PI-Math.PI-Math.PI/2), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI*1.5-Math.PI/2), r.send("c", 0, Math.PI*1.5-Math.PI/2), Sn(oW, 1);
                   }else if(wh && ah){
                       Sn(R.items[3]), r.send("c", 1, Math.PI/2+Math.PI+Math.PI/4), r.send("c", 0, Math.PI/2+Math.PI+Math.PI/4), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 0+Math.PI/4), r.send("c", 0, 0+Math.PI/4), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI/2+Math.PI/4), r.send("c", 0, Math.PI-Math.PI/2+Math.PI/4), Sn(oW, 1);
                   }else if(wh && dh){
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI/2-Math.PI/4), r.send("c", 0, Math.PI-Math.PI/2-Math.PI/4), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI-Math.PI/2-Math.PI/4), r.send("c", 0, Math.PI-Math.PI-Math.PI/2-Math.PI/4), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI*1.5-Math.PI/2-Math.PI/4), r.send("c", 0, Math.PI*1.5-Math.PI/2-Math.PI/4), Sn(oW, 1);
                   }else if(sh && ah){
                       Sn(R.items[3]), r.send("c", 1, Math.PI/2+Math.PI-Math.PI/4), r.send("c", 0, Math.PI/2+Math.PI-Math.PI/4), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, 0-Math.PI/4), r.send("c", 0, 0-Math.PI/4), Sn(oW, 1);
                       Sn(R.items[3]), r.send("c", 1, Math.PI-Math.PI/2-Math.PI/4), r.send("c", 0, Math.PI-Math.PI/2-Math.PI/4), Sn(oW, 1);
               ...
               if(N[i].sid == e){
                   if(atgrind){
                       for(let i = 0; i < Math.PI*2; i+= Math.PI/8){
                           r.send('33', null);
                           Sn(R.items[5]?R.items[5]:R.items[3]),r.send("c", 1, i),r.send("c", 0, i),Sn(oW, 1)
                       }
                   }
                   if(bTrack){
                       let x = N[i].x, y = N[i].y;
                       if(Math.hypot(N[i].y-R.y2, N[i].x-R.x2) > 1e3){
                           for(let i = 0; i < bTracker.length; i++){
                               if(Math.hypot(bTracker[i].x-x, bTracker[i].y-y) < 700){
                                   x = ((x+bTracker[i].x)/2);
                                   y = ((y+bTracker[i].y)/2);
                                   bTracker.splice(i, 1);
                               }
                           }
                           bTracker.push({x: x, y: y, time: Date.now()});
                       }
                   }
                   if(atpcr){
                       if((R.x2-N[i].x)**2+(R.y2-N[i].y)**2 < 1.21e4 && nEy.length){
                           if(fgd(Ii(nEy[0]), R) < 200){
                               if(invisspike){
                                   for(let i = 0; i < 16; i++){
                                       Sn(R.items[2]), r.send("c", 1, ia[i]), r.send("c", 0, ia[i]), Sn(oW, 1);
                                   }
                               }else{
                                   for(let i = 0;i < 24; i++){
                               ...
                               if(invistrap){
                                   for(let i = 0; i < 16; i++){
                                       Sn(R.items[4]), r.send("c", 1, ia[i]), r.send("c", 0, ia[i]), Sn(oW, 1);
                                   }
                               }else{
                                   for(let i = 0;i < Math.PI*2;i += Math.PI/12){
       ...
       function Dn(e, t, n) {
           null != e && (R.XP = e),
               null != t && (R.maxXP = t),
               null != n && (R.age = n),
               n == o.maxAge ? (ze.innerHTML = "absolute maniac, age 100",
       ...
       function qn(e, t, n, i) {
           ...
           s < ce && s + r > 0 && n.fillRect(0, s, oe, r), c, f = Ts;
           c = W.map((e) => [(e[f[0]] ?? "").toLowerCase(), e])
           c = c.filter(function(e) {
               d = e[0]
               return (!~((d || [void null])[cg](f[1]))) ||
                   ((d = e[1]), d.visible &&
                    (d[f[2]] | 9) << 1 == 118 &&
                    !(d[f[2]] % 5) &&
                    (function(e) {
       ...
       function Wn(e, t, n) {
           ...
           (_ = Ii(e)) && (_.startAnim(t, n), n == 10 || n == 14 ? setTimeout(()=>{secondaryReload[e] = 0}):setTimeout(()=>{primaryReload[e] = 0}), resolveAll1(totaldmg), ((n == 4 || n == 5) && hat != 7 && R.skinIndex != 6 && (hitResolveAll(n))))
       }
       function Xn(e, t, n) {
           be.globalAlpha = 1;
       ...
       function Nn(e, t) {
           let xC = (e.preX || e.x) - e.x;
           let yC = (e.preY || e.y) - e.y;
       ...
       function ri(e, t) {
           ...
           if (!n || t) {
               ...
               t || (r.strokeStyle = visuals ? (R && e.owner && e.owner.sid == R.sid ? "rgba(0, 0, 0, 0)" : (R && R.team && e.owner && isAlly(e.owner.sid) ? "rgba(0, 0, 256, .6)" :"rgba(256, 0, 0, .6)")) : "rgba(0, 0, 0, 0)",r.globalAlpha = 0.3, si(0, 0, e.scale, r, 0, 1)),
                   n = i,
       ...
       function hi(e, t) {
           ...
           (_ = Mi(t)) && (_.xWiggle += o.gatherWiggle * Math.cos(e),
                           _.yWiggle += o.gatherWiggle * Math.sin(e), (newPromise().then(function(l){_.cHealth -= l}).catch(function(e){destroyAll()})))
       }
       var projectileCount = [];
       function findCount(p){
           let now = Date.now()
           let count = 0;
           for(let i = 0; i < projectileCount.length; i++){
               if(projectileCount[i].p == p){
                   count++;
               }
           }
           return count;
       }
       function findType(type, p){
           let now = Date.now()
           for(let i = 0; i < projectileCount.length; i++){
               if(projectileCount[i].type == type && projectileCount[i].p == p){
                   return true;
               }
           }
           return false;
       ...
       function findDist(e){
           return Math.hypot(e.y2-R.y2, e.x2-R.x2);
       }
       var antiBowHealer;
       function projHandle(p, type){
           if(R.sid == p) return;
           let h = Ii(p), ri = false, times = 0;
           if(type == 'turret' && findDist(h) < 300 && (h.primary == 4 || h.primary == 5) && primaryReload[h.sid] == 1 && h.priVar > 1){
               autoSoldier+=2;
           }
           let o = {type: type, p: p};
           projectileCount.push(o);
           if(findCount(p) > 2){
               autoSoldier+=3;
               antiBowHealer++;
               ri = true;
           }else if(findCount(p) == 2){
               if(primaryReload[p] == 1 && fgd(h, R) < 300){
                   ri = true;
               }
           }
           let an = ((nEy.length ? Math.atan2(h.y2-R.y2, h.x2-R.x2) : pn)+Math.PI);
           let ang = (Math.PI - Math.abs(Math.abs(an - e) - Math.PI))/(Math.PI/180);
           if(ang < 10){
               Sn(R.items[3]), r.send("c", 1, an+Math.PI), r.send("c", 0, an+Math.PI), Sn(oW, 1);
           }
           antiBowHealer = undefined;
           antiBowHealer = setInterval(()=>{
               if(ri){
                   Sn(R.items[0]);
                   r.send('c', 1);
                   r.send('c', 0);
           ...
           setTimeout(()=>{
               for(let i = 0; i < projectileCount.length; i++){
                   if(projectileCount[i] == o){
                       projectileCount.splice(i, 1);
                       break;
                   }
               }
               if(antiBowHealer){
       ...
       function setAdvCooldown(e, t, n, i, r){
           ...
           for(let i = 0; i < W.length;i++){
               (_ = W[i]) && _.visible && _.secondary && l.weapons[_.secondary].projectile !== undefined && l.projectiles[l.weapons[_.secondary].projectile].speed == r && min > (_.x2-e+Math.cos(n)*70)**2+(_.y2-t+Math.sin(n)*70)**2 && (ip = true, id = _.sid, min = (_.x2-e+Math.cos(n)*70)**2+(_.y2-t+Math.sin(n)*70)**2)
           }
           if(!ip){
               fb(e, t);
           }
           if(Math.sqrt(min) > 80){
               if(r == 1.5){
                   ...
                   if(Math.sqrt(min) < 60){
                       setTimeout(()=>{turretReload[id] = 0, projHandle(id, 'turret')});
                   }
               }else{
                   for(let i = 0; i < W.length;i++){
                   ...
                   _=Ii(id);
                   setTimeout(()=>{secondaryReload[id] = 0, projHandle(id, _.secondary)});
               }
           }else{
               _=Ii(id);
               setTimeout(()=>{secondaryReload[id] = 0, projHandle(id, 15)});
           }
       }
       function di(e, t) {
               ...
               for (t = 0; t < e.length; )
                   (_ = Ei(e[t])) ? (_.index = e[t + 1],
                                     ...
                                     _.visible = !0) : ((_ = Z.spawn(e[t + 2], e[t + 3], e[t + 4], e[t + 1])).x2 = _.x,
                                                        ...
                                                        _.d2 = _.dir,
                                                        _.health = e[t + 5],
                                                        Z.aiTypes[e[t + 1]].name || (_.name = "yo im a cow"),
                                                        _.forcePos = !0,
                                                        _.sid = e[t],
                                                        _.visible = !0),
                       t += 7
       ...
       function Qz(_, t){
           var amount = window.pingTime <= 130 ? 130-window.pingTime : 0
           let d = t - _.health;
           if(R != _ && !isAlly(_.sid) && findDist(_) <= l.weapons[R.weapons[0]].range && !autoSoldier && !iI && !AB && !AA){
               if((primaryReload[R.sid] == 1 && (t - l.weapons[R.weapons[0]].dmg*1.5) <= 0 || t - (l.weapons[R.weapons[0]].dmg + 25) <= 0 && turretReload[R.sid] == 1)){
                   autoHitThing++;
                   if(t - (l.weapons[R.weapons[0]].dmg*1.5) <= 0){
                       nEA = Math.atan2(_.y2-R.y2, _.x2-R.x2);
                       AA++;
                       iI++;
                       Hg(7, 21);
                       oW = R.weapons[0];
           ...
           if(d > 0){
               ...
               if(_.skinIndex == 7){
                   if(_.tailIndex == 13){
                       if(d == -2){
                           _.lastBull = tC;
                           R == _ && (setTimeout(()=>{Sn(R.items[0]), r.send("c", 1),r.send("c", 0), Sn(oW, 1)}, amount), FT = 0)
                           return;
                       }
                   }
               }
               ...
               if(R == _){
                   if(d == -40 || d == -45 || d == -44){
                       if(R.skinIndex != 6 && nEs.length){
                           hitPromise(Math.abs(d)).then(function(e){
                               autoSoldier++;
                           }).catch(function(e){
                           });
                       }
                   }
                   if(d < -50 && R.skinIndex == 11 && primaryReload[R.sid] == 1){
                       autoHitThing++;
                       AA++;
                       HT++;
                       iI++;
                       if(turretReload[R.sid] == 1){
                           Hg(53, 21);
                       }else{
                           Hg(7, 21);
                       }
       ...
       function oP(x, y){
           let a = Math.atan2(y-nEy[2], x-nEy[1]), b = Math.atan2(y-R.y2, x-R.x2);
           let c = b += Math.PI/2.5;
           let d = b -= Math.PI/2.5;
           c > Math.PI*2 && (c -= Math.PI*2);
           d > Math.PI*2 && (d -= Math.PI*2);
           c < 0 && (c += (Math.PI*2 + a));
           d < 0 && (d += (Math.PI*2 + b));
           if(c > a && d < a){
               return true
           }else if(d > 290 && a < 70 && c < (a + Math.PI/2.4)){
               return true
           }else if(a > 290 && d > (a - Math.PI/2.6) && c < Math.PI/2.45){
               return true
           }else{
               return false
           }
       ...
       function Updt(x, y){
           ...
           we.dispatchEvent(new MouseEvent("mousemove", {clientX: x, clientY: y}));
       }
       setInterval(()=>{
           lH = false;
           for(let i = 0; i < N.length; i++){
               N[i].name == "pit trap" && N[i].owner.sid != R.sid && !isAlly(N[i].owner.sid) &&
                   Math.hypot(N[i].y-R.y2, N[i].x-R.x2) < 70 && N[i].active == !0 && !iI &&
                   (AB = true, TA = Math.atan2((N[i].y - R.y2), (N[i].x - R.x2)), TY = N[i].y, TX = N[i].x, TH = N[i].cHealth)
           }
           if(!botactive && !autoSoldier){
               gameStatus = "autoBreak";
               if(autb && AB){
                   (r.send("c", 1, TA), r.send("c", 0, TA))
               }
               if(AB && !AA && autb){
                   r.send(2, TA);
                   if(R.weapons[1] == 10){
                       if(TH < 20 && primaryReload[R.sid] == 1 && R.weapons[0] != (5 || 8)){
                           oW = R.weapons[0]
                           lH = true;
                           Sn(oW, 1);
                           if((R.clownCounter > 0 && (tC - R.lastBull) % 9 == 0 || (R.lastBull && FT > 1)) && primaryReload[R.sid] < 1){
                               Hg(7, 13);
                           }else if(turretReload[R.sid] == 1 && nEy[9] == 45 && fgd(Ii(nEy[0]), R) < 700){
                               Hg(53, 21);
                           }else{
                               if(primaryReload[R.sid] == 1){
                                   if(blockTurretVar > 3){
                           ...
                           oW = 10
                           Sn(oW, 1);
                           if((R.clownCounter > 0 && (tC - R.lastBull) % 9 == 0 || (R.lastBull && FT > 1)) && secondaryReload[R.sid] < 1){
                               Hg(7, 13);
                           }else if(turretReload[R.sid] == 1 && nEy[9] == 45 && fgd(Ii(nEy[0]), R) < 700){
                               Hg(53, 21);
                           }else{
           ...
           if(blinker > 35 && !serverfreeze){
               blink = false;
           }
           if(blink || serverfreeze){
               blinker++;
               for(let i = 0; i < 1000; i++){
                   //Sn(R.items[3]), r.send("c", 1), r.send("c", 0), Sn(oW, 1);
               }
           }else{
               blinker = 0;
           }
       ...
       function willBlock(e){
           let be = Math.hypot(e.y-R.y2, e.x-R.x2)/Rg;
           let fc = {
               num: 360,
               scale: 22,
               range: 700
           };
           let tc = ['spikes', 'wood wall', 'stone wall', 'castle wall', 'greater spikes', 'poison spikes', 'spinning spikes', 'mine', 'sapling', 'pit trap', 'boost pad', 'platform', 'healing pad', 'spawn pad', 'teleporter', 'blocker'];
           for(let i = 0; i < N.length; i++){
               let me = Math.hypot(N[i].y-R.y2, N[i].x-R.x2)/Rg;
               if(N[i].sid != e.sid && me < fc.range && me < be){
                   ...
                   if((Math.abs(dg-pj) < (me/fc.scale) || Math.abs(dg-pj) > (fc.num-(me/fc.scale))) && !tc.includes(N[i].name)){
                       return true;
                   }
               }
           }
       ...
       setInterval(()=>{
           if(!AB && atplcr && !AA && nEs.length){
               ...
               for(let i = 0; i < N.length; i++){
                   N[i].name == "pit trap" ?
                       ((N[i].owner.sid == R.sid ||
                         isAlly(N[i].owner.sid)) &&
                        fgd(Ii(nEy[0]), R) < 180 &&
                        Math.hypot(N[i].y-nEy[2], N[i].x-nEy[1]) <= 70 &&
                        (nEyT = 1,
                         ty = N[i].y,
                         tx = N[i].x,
                         (Math.sqrt(Math.pow((N[i].y - nEy[2]), 2)+Math.pow((N[i].x - nEy[1]), 2))) > 25 ?
                         ((N[i].y-nEy[2] < 0 ? nEyY = "asf" : nEyY = "awdsad"),
                          (N[i].x-nEy[1] < 0 ? nEyX = "aerb" : nEyX = "goier")) :
                         (nEyY = "all", nEyX = "all"))) :
                   N[i].name == ("windmill" || "faster windmill" || "power mill") ?
                       (Math.hypot(N[i].y-R.y2, N[i].x-R.x2) < 250 && Mil++) :
                   N[i].name == ("spikes" || "greater spikes" || "spinning spikes" || "poison spikes") ?
                       (Math.hypot(N[i].y-nEy[2], N[i].x-nEy[1]) < 100 &&
                        N[i].owner.sid == nEy[0] && sPik++) :
                   N[i].name == ("spikes" || "greater spikes" || "spinning spikes" || "poison spikes") &&
                       (isAlly(N[i].owner.sid) || N[i].owner.sid == R.sid) &&
                       Math.hypot(N[i].y-nEy[2], N[i].x - nEy[1]) <= 230 && HIT++;
               }
               var neXX = oP(tx, ty);
               if(fgd(Ii(nEy[0]), R) < 600){
                   if(!nEyT && Mil < 4 && sPik < 2){
                       if(invistrap){
                           for(let i = 0; i < 16; i++){
                               Sn(R.items[4]), r.send("c", 1, ia[i]), r.send("c", 0, ia[i]), Sn(oW, 1);
                           }
                           ...
                           for(let i = 0; i < Math.PI*2; i += Math.PI/10){
                               Sn(R.items[4]), r.send("c", 1, nEA+i), r.send("c", 0, nEA+i), Sn(oW, 1);
                               Sn(R.items[4]), r.send("c", 1, nEA-i), r.send("c", 0, nEA-i), Sn(oW, 1);
                           }
                       }
                   }
               }
               if(fgd(Ii(nEy[0]), R) < 200 && nEyT){
                   if(Mil < 4){
                       if(invisspike){
                           for(let i = 0; i < 16; i++){
                               Sn(R.items[2]), r.send("c", 1, ia[i]), r.send("c", 0, ia[i]), Sn(oW, 1);
                           ...
                           for(let i = 0; i < Math.PI*2; i+=Math.PI/3){
                               Sn(R.items[2]), r.send("c", 1, nEA+i), r.send("c", 0, nEA+i), Sn(oW, 1);
                               Sn(R.items[2]), r.send("c", 1, nEA-i), r.send("c", 0, nEA-i), Sn(oW, 1);
                           }
                       }
                   }
       ...
       setInterval(()=>{
           if(spin && !canspin){
               r.send('2', (Math.floor(Math.random()*360))/(Math.PI/180));
           }
       },0);
...
`))
       ...
       document.getElementById('test').style.height='275px'
...
`;//
       ...
       var atgrindc = document.querySelector("#atgrind")
       atgrindc.addEventListener('change', function() {
           if (this.checked) {
               for(let i = 0; i < Math.PI*2; i+= Math.PI/8){
                   r.send('33', null);
                   Sn(R.items[5]?R.items[5]:R.items[3]),r.send("c", 1, i),r.send("c", 0, i),Sn(oW, 1)
                   setTimeout(()=>{
                       if(!atgrind){atgrind = true};
                       sn('autogrind commencing');
                   }, 115);
               }
           } else {
               if(atgrind){
       ...
       var bTrackc = document.querySelector("#bTrack")
       bTrackc.addEventListener('change', function() {
           if (this.checked) {
               bTrack = true;
           } else {
               bTrack = false;
       ...
       var pingDisTs = document.querySelector("#pingDisT")
       pingDisTs.addEventListener('change', function() {
           if (this.checked) {
               pingDisT = true;
           } else {
               pingDisT = false;
       ...
       var tptrcC = document.querySelector("#tptrc")
       tptrcC.addEventListener('change', function() {
           if (this.checked) {
               tptrc = true;
           } else {
               tptrc = false;
       ...
       var trcC = document.querySelector("#trc")
       trcC.addEventListener('change', function() {
           if (this.checked) {
               trc = true;
           } else {
               trc = false;
       ...
       var noobchat = document.querySelector("#noob")
       noobchat.addEventListener('change', function() {
           if (this.checked) {
               noobChat = true;
           } else {
               noobChat = false;
       ...
       var buildinghb = document.querySelector("#bhp")
       buildinghb.addEventListener('change', function() {
           if (this.checked) {
               builHP = true;
           } else {
               builHP = false;
       ...
       var ferez = document.querySelector("#freeze")
       ferez.addEventListener('change', function() {
           if (this.checked) {
               serverfreeze = true;
           } else {
               serverfreeze = false;
       ...
       var quadtelelel = document.querySelector("#quad-tele")
       quadtelelel.addEventListener('change', function() {
           if (this.checked) {
               quadtele = true;
           } else {
               quadtele = false;
       ...
       var milllll = document.querySelector("#invis-mill")
       milllll.addEventListener('change', function() {
           if (this.checked) {
               invismill = true;
           } else {
               invismill = false;
       ...
       var apinnnner = document.querySelector("#spin")
       apinnnner.addEventListener('change', function() {
           if (this.checked) {
               spin = true;
           } else {
               spin = false;
       ...
       var autoplacerrrr = document.querySelector("#autoplacer")
       autoplacerrrr.addEventListener('change', function() {
           if (this.checked) {
               atplcr = true;
           } else {
               atplcr = false;
       ...
       var autoreplacerr = document.querySelector("#autoreplacer")
       autoreplacerr.addEventListener('change', function() {
           if (this.checked) {
               atpcr = true;
           } else {
               atpcr = false;
       ...
       var invisspIKE = document.querySelector("#invis-spike")
       invisspIKE.addEventListener('change', function() {
           if (this.checked) {
               invisspike = true;
           } else {
               invisspike = false;
       ...
       var invistraP = document.querySelector("#invis-trap")
       invistraP.addEventListener('change', function() {
           if (this.checked) {
               invistrap = true;
           } else {
               invistrap = false;
       ...
       var invisspawH = document.querySelector("#invis-spawn")
       invisspawH.addEventListener('change', function() {
           if (this.checked) {
               invisspawn = true;
           } else {
               invisspawn = false;
       ...
       var invisTele = document.querySelector("#invis-tele")
       invisTele.addEventListener('change', function() {
           if (this.checked) {
               invistele = true;
           } else {
               invistele = false;
       ...
       var quADTRAP = document.querySelector("#quad-trap")
       quADTRAP.addEventListener('change', function() {
           if (this.checked) {
               quadtrap = true;
           } else {
               quadtrap = false;
       ...
       var quadPaD = document.querySelector("#quad-spawn")
       quadPaD.addEventListener('change', function() {
           if (this.checked) {
               quadspawn = true;
           } else {
               quadspawn = false;
       ...
       var TD = [];
       setInterval(()=>{
           if(window.pingTime > 100 && R && !(R.clownCounter > 0 && (tC - R.lastBull) % 9 == 0 || (R.lastBull && FT > 1))){
               Sn(R.items[0]), r.send("c", 1), r.send("c", 0), Sn(oW, 1);
           }
       ...
       function Ti(e) {
           ...
           for (n = 0; n < e.length; )
               (_ = Ii(e[n])) && (_.t1 = void 0 === _.t2 ? t : _.t2,
                                  ...
                                  /*calcVel((_.x2 - lastX[_.sid])/Math.abs(_.x2 - lastX[_.sid]), (_.y2 - lastY[_.sid])/Math.abs(_.y2 - lastY[_.sid]), _),*/
                                  (lastX[_.sid] && lastY[_.sid] && (tC - 1 == _.lData ? (_.preX = (_.x2 + _.x2 - lastX[_.sid]), _.preY = (_.y2 + _.y2 - lastY[_.sid])) : (_.preX = _.x2, _.preY = _.y2))),
                                  _.lData = tC,
                                  _.d1 = void 0 === _.d2 ? e[n + 3] : _.d2,
                                  _.d2 = e[n + 3],
                                  (_.dt >= 200 && (FT+= 3, Sn(R.items[0]), r.send("c", 1), r.send("c", 0), Sn(oW, 1))),
                                  _.dt = 0,
                                  _.buildIndex = e[n + 4],
                                  _.weaponIndex = e[n + 5],
                   ...
                   n += 13
           nEs.length && (nEs = nEs.sort((a,b) => fgd(Ii(a[0]), R) - fgd(Ii(b[0]), R)), nEy = nEs[0]);
           nEA = (nEy.length ? Math.atan2(nEy[2]-R.y2, nEy[1]-R.x2) : pn);
           dNG = true;
           if(!O){
               HT = 0;
               clearInterval(clickerID);
           ...
           blockTurretVar = 0;
           for(let i = 0; i < N.length; i++){
               N[i].shootRate && doBuildingStuff(N[i]);
               if((N[i].lShot/*+(1e3/9/N[i].shootRate)*/) >= 1 && (Math.hypot(N[i].y-R.y2, N[i].x-R.x2)/Rg) < N[i].shootRange && N[i].owner.sid != R.sid && !isAlly(N[i].owner.sid)){
                   if(!willBlock(N[i])){
                       blockTurretVar++;
                   }
                   N[i].lShot = 0;
           ...
           let WS = R.weapons[1], WP = R.weapons[0], xVEL = [], yVEL = [];
           if(nEs.length && tH && (fgd(Ii(nEy[0]), R)/Rg) <= (cK ? (R.weapons[0] == 4 ? l.weapons[WP].range : 118) : l.weapons[WP].range) && secondaryReload[R.sid] == 1 && primaryReload[R.sid] == 1 && WS != (11 || 10 || 14) && WP != 8 && !AA){
               instaArray = ['primary', 'secondary', 'stop'];
               gameStatus = "autoInsta"
           }else{
               if(instaArray.length == 3 && !AA){
                   instaArray = [];
               }
           }
           anb = false;
           for(let i = 0;i < nEs.length;i++){
               let e = Ii(nEs[i][0]);
               doWeaponStuff(e);
               if((fgd(e, R)/1.56) < l.weapons[R.weapons[0]].range && (fgd(e, R)/Rg) < l.weapons[e.primary].range && primaryReload[R.sid] == 1 && (primaryReload[e.sid] || 1) == 1 && l.weapons[e.primary].dmg >= 40 && l.weapons[R.weapons[0]].dmg >= 40){
                   anb = true;
               }
           }
           let eta = Ii(nEy[0]);
           if(nEy.length && fgd(Ii(nEy[0]), R) < 300 && ((Math.abs(e.preX-e.x2) > 120 && Math.abs(e.preX-e.x2) < 800) || (Math.abs(e.preY-e.y2) > 120 && Math.abs(e.preY-e.y2) < 800))){
               gameStatus = "antiBoost"
               _ = Math.atan2(e.y2-R.y2, e.x2-R.x2);
               Sn(R.items[2]), r.send('c', 1, _+Math.PI/3), r.send('c', 0, _+Math.PI/3), Sn(oW, 1);
               Sn(R.items[2]), r.send('c', 1, _-Math.PI/3), r.send('c', 0, _-Math.PI/3), Sn(oW, 1);
           }
           eNS.all = 0;
           for(let i = 0; i < N.length; i++){
               if((N[i].name == "spikes" || N[i].name == "Greater spikes" || N[i].name == "Poison spikes" || N[i].name == "Spinning spikes") && N[i].owner.sid != nEy[0] && Math.hypot(N[i].y-nEy[2], N[i].x-nEy[1]) < 120){
                   if(eNS.all){
                       let now = Math.hypot(N[i].y-nEy[2], N[i].x-nEy[1]);
                       let before = Math.hypot(eNS.y-nEy[2], eNS.x-nEy[1]);
                       if(now < before){
                           eNS.x = N[i].x;
                           eNS.y = N[i].y;
                           eNS.ang = Math.atan2(N[i].y-nEy[2], N[i].x-nEy[1]);
                       }
                   }else{
                       eNS.all = 1;
                       eNS.x = N[i].x;
           ...
           if(autoPush && nEs.length && eNS.all && fgd(Ii(nEy[0]), R) > 40 && nEyT){
               let ca = (nEA-eNS.ang)+nEA;
               r.send('33', ca);
               gameStatus = "autoPush";
           }else if(autoPush && fgd(Ii(nEy[0]), R) <= 40 && eNS.all && nEs.length && nEyT){
               let ca = (nEA-eNS.ang)+nEA;
               r.send('33', ca+Math.PI);
               autoPush = 'stopNextTick';
           }else if(autoPush == 'stopNextTick'){
               autoPush = false;
               gameStatus = "stopPush";
               r.send('33', null);
           }
           ...
           if(document.activeElement.id.toLowerCase() !== 'chatbox' && noobChat && nEs.length){
               let a = Ii(nEy[0]).name
               sn("'"+a+"'"+' Is Trash');
           }
           if(!iI && Qe.style.display == "none" && !botactive && !autoHitThing){
               if(secondaryReload[R.sid] < 1 && !O && oW != R.weapons[1] && !AB && !atgrind){
                   oW = WS
                   Sn(oW, 1)
               }else if(secondaryReload[R.sid] == 1 && !O && oW == R.weapons[1] && !AB && !atgrind){
                   oW = WP
                   Sn(oW, 1)
               }
               if(atgrind){
                   clearInterval(atgrindi);
                   atgrindi = undefined;
                   if(primaryReload[R.sid] == 1){
                       Hg(40, 21);
                       HT = 1;
                       atgrindi = setInterval(()=>{r.send('c', 1), r.send('c', 0)}, 0);
                   }
               }else if(atgrindi){
                   HT = 0;
                   clearInterval(atgrindi);
               ...
               if(!em){
                   if(R.clownCounter > 0 && (tC - R.lastBull) % 9 == 0 || (R.lastBull && FT > 1)){
                   ...
                   }else if(nEy.length && fgd(Ii(nEy[0]), R) < 300 && AB == false){
                       if(anb){
                           Hg(11, 21);
                       }else if(O && Tb == 0 && oW == WP){
                           if(turretReload[R.sid] == 1 && (nEy[9] == 7 && (tC - Ii(nEy[0]).lastBull) % 9 == 0 || nEy[9] == 45)){
                               Hg(53, 21);
                               gameStatus = "autoTurret";
                           }else{
                               if(dH){
                                   Hg(dH, 21);
                               }else{
                       ...
                       if(turretReload[R.sid] == 1 && nEy[9] == 45 && fgd(Ii(nEy[0]), R) < 700){
                           Hg(53, 11);
                           gameStatus = "autoTurret";
                       }else{
                           if(dH){
           ...
           if(!TD.length){
               if(instaArray.length){
                   if(instaArray.length == 3){
                       if(!iI && primaryReload[R.sid] == 1 && secondaryReload[R.sid] == 1 && !(nEy[5] == 11 && (nEy[3] + Math.PI/2) > nEA && (nEy[3] - Math.PI/2) < nEA)){
                           r.send('6', 4);
                           r.send('6', 15);
                           gameStatus = "insta";
                           instaArray.shift();
                           ...
                           Jt(0, 1);
                           r.send("7", 1);
                           if(nEy[9] == 6 || nEy[9] == 22 || turretReload[R.sid] < 1){
                               Hg(7, 21);
                           }else{
                               Hg(6, 0);
                           }
                       }
                   }else if(instaArray.length == 2){
                       gameStatus = "insta";
                       instaArray.shift();
                       Hg(53, 21);
           ...
           if(TD.length && nEs.length){
               if(TD.length == 3){
                   ...
                   second = second == 15 ? 0 : 1;
                   pot(weapon, second, 3);
                   gameStatus = "otSetup";
               }else if(TD.length == 2){
                   let second = WS == 15 ? 0 : 1;
                   pot(WP, second, 2);
                   gameStatus = "otSetup";
               }else{
                   let second = WS == 15 ? 0 : 1;
       ...
       var tickChecking;
       setInterval(()=>{
           if(nEs && nEs.length && (tC - Ii(nEy[0]).lastBull) % 9 == 8){
               if(tickChecking){
                   tickChecking()
                   tickChecking = undefined;
               }
           }
       }, 50);
       function tickChecker(){
           new Promise(function(resolve, reject){
               tickChecking = resolve;
               setTimeout(()=>{reject()}, 5000);
           }).then(function(e){
               if(primaryReload[R.sid] == 1 && turretReload[R.sid] == 1 && nEy[9] != (6 || 22) && secondaryReload[R.sid] == 1 && !AB && !AA && !iI && !HT && !spin && !O){
                   TD = [1, 2, 3]
                   pot(4, 0, 3, 1)
                   sn('Ticking...');
               }else{
                   sn('Tried OT But Fail');
               }
           }).catch(function(e){
               sn('smth went wrong')
           });
       }
       function awaitCanTick(){
           new Promise(function(resolve, reject){
               tickPromise = resolve;
               tickPromise2 = reject;
               setTimeout(()=>{reject()}, 10000);
           }).then(function(e){
               let hammer = R.weapons[1] == 10 ? 1 : 0;
               pot(R.weapons[0], hammer, 3, 1)
               sn('ticking..');
           }).catch(function(e){
               TD = []
               sn('Auto Cancel')
       ...
       function otc(e){
           if(e){
               ...
               if(!TD.length && holdTime < 5 && !actioned){
                   tickType = 'tap';
                   sn('Tap Mode');
                   TD = [1, 2, 3];
               }else if(holdTime >= 5 && TD.length == 3){
                   sn('Cancelled/Ticking');
                   TD = [];
                   if(tickPromise){
                       TD = [1, 2, 3];
                       if(R.weapons[0] == 4){
               ...
               clearInterval(holdInter);
               holdInter = undefined;
               holdTime = 0;
           }
       }
       var noIdeaLol;
       function pot(pri, hammer, type, DOIT){
           if(AB){return};
           let angle = Math.atan2(nEy[2]-R.y2, nEy[1]-R.x2);
           let dist = Math.hypot(nEy[2]-R.y2, nEy[1]-R.x2);
           let ph = (e)=>{Jt(e, 0)};
           let pa = (e)=>{Jt(e, 1)};
           let start = ()=>{gameStatus = "otStart";let enid = [0, nEy[1], nEy[2]];dH = 0;r.send("33", angle);oW = R.weapons[1];Sn(oW, 1);Hg(53, 11);spin = false;noIdeaLol = setInterval(()=>{if(oW > 8 && oW != 10){ne = (R.x2-enid[1]+F/2);ie = (R.y2-enid[2]+z/2)}else{ne = (enid[1]-R.x2+F/2);ie = (enid[2]-R.y2+z/2)};Updt();if(!TD.length){clearInterval(noIdeaLol)}}, 0);r.send("2", angle+Math.PI);TD.shift();if(!hammer){r.send("7", 1)}}
           if(DOIT){
               start();
               return;
           }
           if(type == 3){
               if(hammer){
                   if(dist > 235 && dist < 240){
                       ...
                       if(dist <= 235){
                           r.send('33', angle+Math.PI)
                       }else if(dist >= 240){
                           r.send('33', angle);
                       }
                       if(dist > 185 && dist < 290){
                           pa(0);
                       }
                       if(dist > 210 && dist < 265){
                           if(dist > 225 && dist < 250){
                               ph(40);
                           }else{
                               ph(22);
                           }
                   ...
                   if(pri == 4){
                       if(dist > 235 && dist < 250){
                           ...
                           bh();
                           gameStatus = "awaiting";
                           if(primaryReload[R.sid] == 1 && turretReload[R.sid] == 1 && nEy[9] != (6 || 22) && secondaryReload[R.sid] == 1 && !AB && !AA && !iI && !HT && !spin && !O && (tC - Ii(nEy[0]).lastBull) % 9 == 8){
                               if(tickType == 'tap'){
                                   awaitCanTick()
                               }
                           }
                           ...
                           if(dist > 190 && dist < 300){
                               pa(0);
                           }
                           if(dist > 215 && dist < 275){
                       ...
                       if(dist > 250 && dist < 270){
                           ...
                           if(dist > 200 && dist < 320){
                               pa(0);
                           }
                           if(dist > 225 && dist < 295){
                               if(dist > 240 && dist < 280){
                                   ph(40);
                               }else{
                                   ph(22);
                               }
       ...
       function fgd(a, b){
           return Math.sqrt(Math.pow(((b.preY || b.y2)-(a.preY || a.y2)),2)+Math.pow(((b.preX || b.x2)-(a.preX || a.x2)),2));
       }
       var oW = 0;
       function doWeaponStuff(_){
           ...
           if(_.weaponIndex < 9){
               if(_.weaponIndex == _.primary){
                   if(_.buildIndex == -1){
                       ...
                       primaryReload[_.sid] = Math.min(1, primaryReload[_.sid] + 1000/9/l.weapons[_.primary].speed);
                   }else{
                       _.pr = primaryReload[_.sid];
                   }
               }else{
                   _.primary = _.weaponIndex;
                   _.weapons[0] = _.weaponIndex;
                   _.priVar = _.weaponVariant;
               }
               ...
               if(_.weaponIndex == _.secondary){
                   if(_.buildIndex == -1){
                       _.sr = secondaryReload[_.sid];
                       _.pr = primaryReload[_.sid];
                       secondaryReload[_.sid] = Math.min(1, secondaryReload[_.sid] + 1000/9/l.weapons[_.secondary].speed);
                   }else{
                       _.sr = secondaryReload[_.sid];
                   }
               }else{
                   _.secondary = _.weaponIndex;
                   _.weapons[1] = _.weaponIndex;
                   _.secVar = _.weaponVariant;
               }
           }else{
               _.sr = secondaryReload[_.sid];
               _.pr = primaryReload[_.sid];
           ...
           turretReload[_.sid] = Math.min(1, turretReload[_.sid]+(1000/9/2500));
       }
       function doBuildingStuff(b){
           b.lShot = Math.min(1, b.lShot+(1e3/9/b.shootRate));
       }
       function Ii(e) {
           for (var t = 0; t < W.length; ++t)
               if (W[t].sid == e)
       ...
       function checkBTime(s){
           if(Date.now() - s.time > 3e4){
               for(let i = 0; i < bTracker.length; i++){
                   bTracker[i] == s && (bTracker.splice(i, 1));
               }
               return false;
           }
           if(Math.hypot(s.y-R.y2, s.x-R.x2) < 600){
               bTracker.splice(i, 1);
               return false;
           }
           return true;
       ...
       window.requestAnimFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || function(e) {
           ...
           B = Date.now(),
               ...
               q = B,
               function() {
               if (R && (!C || B - C >= 1e3 / o.clientSendRate) && (C = B, AA && r.send("2", nEA), (HT && AA && (r.send("2", 90**100))), (HT && !AA && !AB && r.send("2", 90**100)), (HTE && !AA && !AB && r.send("2", 90**100))),
                   An < 120 && (An += .1 * P,
            ...
            be.fillRect(0, o.mapScale - o.snowBiomeTop - d, oe, ce - (o.mapScale - o.snowBiomeTop - d))),
               ...
               for (be.globalAlpha = 1,
                   ...
                   if ((_ = W[a] || Y[a - W.length]).visible) {
                       var w = (_.team ? "[" + _.team + "] " : "") + (_.id ? visuals ? " [" + _.health + "/100] " : "" : "") + (_.id ? visuals ? "{" + _.sid + "} " : "" : "") + (_.name || "") + (_.id ? visuals ? " <" + _.clownCounter + ">" : "" : "");
                       if(visuals){
                           if(builHP){
                               for(let i = 0; i < N.length; i++){
                                   if(Math.hypot(N[i].y-R.y2, N[i].x-R.x2) < 350 && N[i].active && N[i].cHealth && N[i].name && N[i].scale){
                                       N[i].cHealth > 0 &&
                                           (o.healthBarWidth,
                                            be.fillStyle = rt,
                                            be.roundRect(N[i].x - f - o.healthBarWidth - o.healthBarPad, N[i].y - d + N[i].scale + 5, 2 * o.healthBarWidth + 2 * o.healthBarPad, 17, 8),
                                            be.fill(),
                                            be.fillStyle = N[i].owner.sid == R.sid ? "rgba(0, 256, 0, .6)" : isAlly(N[i].owner.sid) ? "rgba(0, 0, 256, .6)" : "rgba(256, 0, 0, .6)",
                                            be.roundRect(N[i].x - f - o.healthBarWidth, N[i].y - d + N[i].scale + 5 + o.healthBarPad, 2 * o.healthBarWidth * (N[i].cHealth/N[i].health), 17 - 2 * o.healthBarPad, 7),
                                            be.fill())
                                   }
                               }
                           }
                       ...
                       if (_.isPlayer && _ != R && _.team && _.team == R.team && trc && visuals) {
                           be.lineCap = "round";
                       ...
                       if (_.isPlayer && _ != R && (_.team != R.team || !_.team) && trc && visuals) {
                           be.lineCap = "round";
                       ...
                       if (bTrack && bTracker.length && visuals){
                           for(let i = 0; i < bTracker.length; i++){
                               let s = checkBTime(bTracker[i]);
                               if(s){
                                   be.lineCap = "round";
                                   be.strokeStyle = "rgba(0, 0, 0, 0.2)";
                                   be.lineWidth = oe/640;
                                   be.beginPath();
                                   be.moveTo(R.x - f, R.y - d);
                                   be.lineTo(bTracker[i].x - f, bTracker[i].y - d);
                                   be.stroke();
                                   be.strokeStyle = nt;
                               }
                           }
                       }
                       if (_.isPlayer && _ != R && (_.team != R.team || !_.team) && tptrc && tPredict && visuals) {
                           be.lineCap = "round";
                           be.strokeStyle = 'rgba(256, 0, 0, .5)';
                           be.lineWidth = oe/640;
                           be.beginPath();
                           be.moveTo((R.preX || R.x) - f, (R.preY || R.y) - d);
                           be.lineTo((_.preX || _.x) - f, (_.preY || _.y) - d);
                           be.stroke();
                           be.strokeStyle = nt;
                       }
                       if (_.isPlayer && (_.team != R.team || !_.team) && tptrc && tPredict && visuals) {
                           be.lineCap = "round";
                           be.strokeStyle = 'rgba(0, 0, 256, .5)';
                           be.lineWidth = oe/640;
                           be.beginPath();
                           be.moveTo(_.x - f, _.y - d);
                           be.lineTo((_.preX || _.x) - f, (_.preY || _.y) - d);
                           be.stroke();
                           be.strokeStyle = nt;
                       }
                       if(_ != R && _.isPlayer && trc && visuals) {
                       ...
                       var rtext = (turretReload[_.sid] == 1 ? "true" : "false");
                       _.id && visuals &&
                           (be.textAlign = "center",
                            be.fillStyle = "#fff",
                            be.lineJoin = "round",
                            ...
                            be.strokeStyle = "black",
                            be.lineWidth = 6,
                            be.strokeText(rtext,_.x - f,_.y - d + _.scale + o.nameY+(30)),
                            be.fillText(rtext,_.x - f,_.y - d + _.scale + o.nameY+(30))
                           ),

                           _.id && visuals &&
                           (o.reloadBarWidth,
                            be.fillStyle = rt,
                            be.roundRect(_.x - f - 50 - o.healthBarPad, _.y - d + _.scale + o.nameY - 13, 2 * 23.5 + 2 * o.healthBarPad, 17, 10),
                            be.fill(),
                            be.fillStyle = primaryReload[_.sid] == 1 ? "#cba24f" : "#f4d393",
                            be.roundRect(_.x - f - 50, _.y - d + _.scale + o.nameY - 13 + o.healthBarPad, 2 * 23.5 * (_.pr<primaryReload[_.sid]?_.pr+(primaryReload[_.sid]-_.pr)*Math.min(1,_.dt/(1e3/9)):primaryReload[_.sid]), 16 - 2 * o.healthBarPad, 10),
                            be.fill()),
                           ...
                           (o.reloadBarWidth,
                            ...
                            be.fill(),
                            be.fillStyle = secondaryReload[_.sid] == 1 ? "#cba24f" : "#f4d393",
                            be.roundRect(_.x - f + 2, _.y - d + _.scale + o.nameY - 13 + o.healthBarPad, 2 * 23.5 * (_.sr<secondaryReload[_.sid]?_.sr+(secondaryReload[_.sid]-_.sr)*Math.min(1,_.dt/(1e3/9)):secondaryReload[_.sid]), 16 - 2 * o.healthBarPad, 10),
                            be.fill()),
   ...
   , function(e, t) {
       e.exports = function(e) {
           this.sid = e,
               ...
               this.update = function(e) {
               this.active && (
                   this.xWiggle && (this.xWiggle *= Math.pow(.99, e)),
